...
double logistic(double sigma, double var, double delta){
	double value = (sigma*pow(var, 2))/(delta+pow(var, 2));
	return value;
}

double mu1(double s, double b){
	double value = max((pow(s, 1))/(b1+pow(s, 1)), 0.0) * (1-logistic(sigma_b, b, delta_b)) * mu1o;
	return value;
}

double mu2(double s, double r, double g){
	double value = max((pow(s, 1))/(b2+pow(s, 1)), 0.0) * (1-logistic(sigma_r, r, delta_r)+logistic(sigma_g, g, delta_g)) * mu2o;
	return value;
}
...
int main(){
	...
	for(int i=0; i<iter; i++){
		...
		diff_x1 = x1.diff(mu1(s.value, b), kd1, x1.value, pow(x1.value, 2));
		diff_x2 = x2.diff(mu2(s.value, r, g), kd2, x2.value, pow(x2.value, 2));
	...
	//Output Format - t, x1, x2, s, p1, p2, p3, x1/x2
